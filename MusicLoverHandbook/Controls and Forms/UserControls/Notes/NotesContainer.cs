...
namespace MusicLoverHandbook.Controls_and_Forms.UserControls.Notes
{
    public class NotesContainer
    {
        ...
        private NoteAdd CreateAddButton(NoteControlParent parent) => new NoteAdd(parent, $"Add new {parent.NoteType+1}", "Click on me to add new note");

        private void SetupAddNoteButton(NoteControlParent note) => SetupAddNoteButton(note, true, true);
        private void SetupAddNoteButton(NoteControlParent note, bool recursiveForParent, bool recursiveForChildren)
        {
            ...
            if (recursiveForParent)
                if (note is NoteControlMidder midder && midder.ParentNote is NoteControlParent anotherParent)
