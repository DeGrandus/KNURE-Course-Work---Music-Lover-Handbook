    ...
    public partial class InputData : UserControl
    {
        ...
        public OutputInfo GetOutput()
        {
            ...
            if (InputNameBox.Status == InputStatus.ANALOG)
                if (MessageBox.Show($@"Field with name ""{InputNameBox.Text}"" has an already existing analog ""{(string)InputNameBox.Tag}"". If you want to update an already created note, select OK. Otherwise - Cancel.", $@"Field ""{InputType}"" has an analog", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.OK)
