...
namespace MusicLoverHandbook.Controls_and_Forms.Custom_Controls
{
    public class SideButtonsPanel : Panel
    {
        ...
        {
            if (deactivatedButtons.Where(x => x.ButtonType == type).FirstOrDefault() is ButtonPanel button)
            {
                Controls.Add(button);
                deactivatedButtons.Remove(button);
                activatedButtons.Add(button);
        ...
        {
            if (activatedButtons.Where(x=>x.ButtonType==type).FirstOrDefault() is ButtonPanel button)
            {
                Controls.Remove(button);
